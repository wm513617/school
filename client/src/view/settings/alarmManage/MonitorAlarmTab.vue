<template>
  <div class="tab-content-alarm">
    ...
    <div v-if="isOpenAddMod">
      <AddModal :isOpen="isOpenAddMod" addModTitle="监控点报警添加" type="monitorAlarm" :deviceTree="deviceTree" @close="close" @addNewOne="addNewOne"></AddModal>
    </div>
    <!-- 修改 批量修改 弹框 -->
    <div v-if="isOpenEditMod">
      <Modal v-model="isOpenEditMod" :width="480" :title="isEditOne ? '监控点报警修改' : '监控点报警批量修改'" :mask-closable="false">
    ...
    <!-- 联动配置 弹框 -->
    <div v-if="isShowLink">
      <LinkConfigModal :model="isShowLink" :resData="linkData" :rootOrgId="orgId" type="monitor" :activeOrgId="activeOrgId" @close="isShowLink = false" @refrash="getImportData"></LinkConfigModal>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      isOpenAddMod: false,
      deviceTree: [],
      isEditOne: true, // 单个 or 批量修改
      isOpenEditMod: false, // 修改 批量修改 弹框控制
      editDataInfo: {
        orgName: '所属设备',
        chan: 1,
        subtype: '',
  ...
  methods: {
    ...mapActions(['getMonitorTree', 'getMonitorAlarmData', 'addMonitorAlarmData', 'recordLog', 'editMonitorAlarmData', 'editMoreMonitorAlarmData', 'deleteMonitorAlarmData', 'getOneAlarmTabNumber']),
    // 获取报警输入列表数据
    getImportData() {
      let data = {
      ...
      this.getMonitorAlarmData(data)
        .then((res) => {
          this.isCannotClick = true
          this.dataList = res.data
          this.pageTotal = Number(res.headers['x-bsc-count'])
          this.getOneAlarmTabNumber({type: 'monitoryPointAlarmNo', count: this.pageTotal})
        })
        .catch(err => {
          console.log('logout error:' + err)
          this.errorMsg('监控点报警获取失败')
        })
    },
    search() {
      this.pageCur = 1
    ...
    addNewOne(data) {
      ...
      this.addMonitorAlarmData(param).then(res => {
        this.recordMsg(data.resNodes, '报警添加', '监控点报警添加')
        this.getImportData()
        this.close()
      }).catch((err) => {
    ...
    sureDeleteAlarm() {
      ...
      this.deleteMonitorAlarmData(ids).then(res => {
        this.recordMsg(this.selectData, '报警删除', '监控点报警删除')
    ...
    edit(val) {
      if (val) {
        ...
        // this.editDataInfo = Object.assign({}, this.editDataInfo, this.selectData[0])
      ...
      this.$refs['editForm'].validate(valid => {
        if (valid) {
          ...
          if (this.isEditOne) {
            ...
            this.editMonitorAlarmData(param).then(res => {
              this.recordMsg(this.selectData, '报警修改', '监控点报警修改')
              this.getImportData()
            }).catch(() => {
            ...
            this.editMoreMonitorAlarmData(param).then(() => {
              this.recordMsg(this.selectData, '报警修改', '监控点报警批量修改')
              this.getImportData()
            }).catch(() => {
