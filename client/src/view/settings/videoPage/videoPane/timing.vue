<template>
  <div class="timing-box" ref="tableBox">
    ...
    <div class="table-box">
      ...
      <div>
        <Button type="primary" :disabled="!isDisable" class="top-margin-botton" :loading="loading" style="width: 100px" @click="saveTiming">保存</Button>
      </div>
    </div>
  </div>
</template>
...
export default {
      ...
      columns4: [
        ...
        },
        {
          ...
          render: (h, params) => {
            if (Object.getOwnPropertyNames(params.row).length !== 4) {
              var that = this
              var middleCode = params.row.conf ? params.row.conf.streamType : ''
        ...
        },
        {
          ...
          render: (h, params) => {
            // if (Object.getOwnPropertyNames(params.row).length !== 4) {
            var _that = this
            var defaultVideoValue = params.row.conf
  ...
  watch: {
    getSelectTree() {
      if (this.monmentVideoTabs === 'timing') {
        this.getTimeData(1)
      }
    },
    ...
    isUpdate(newval) {
      if (this.monmentVideoTabs === 'timing') {
        this.getTimeData(this.currentPage, true)
      }
    },
    monmentVideoTabs() {
      if (this.monmentVideoTabs === 'timing') {
  ...
  methods: {
    ...
    saveTiming() {
      ...
      this.changeRecordByTree(saveChangeList)
        .then(suc => {
          this.getTimeData(this.currentPage)
          this.$Notice.success({
            title: '提示',
            desc: '参数保存成功',
            duration: 2
        ...
        .catch(err => {
          this.loading = false
          console.log('save recorde err:' + err)
          // this.$Notice.error({
          //   title: '提示',
          //   desc: '参数保存失败',
          //   duration: 2
  ...
  created() {
    this.isDisable = this.$BShasPower('BS-SETTING-VIDEO-TIMING-MANAGE')
    if (this.getSelectTree && this.monmentVideoTabs === 'timing') {
      this.getTimeData(1)
    }
    this.getplanName()
      .then(suc => {
      ...
      .catch(err => {
        console.log('get timeData err' + err)
