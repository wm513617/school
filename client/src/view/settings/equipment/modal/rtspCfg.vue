    ...
    <div>
      <Form ref="rtspCfg" :model="formData" :rules="ruleDeviceForm" :label-width="100" label-position="left" style="padding: 0 20px;">
        <Form-item label="用户名" prop="username">
          <Input v-model="formData.username" :maxlength=64></Input>
        </Form-item>
        <Form-item label="密码" prop="password">
          <Input v-model="formData.password" type="password" :maxlength=64></Input>
        </Form-item>
        <Form-item label="地址" prop="url">
          <Bsipv4 v-model="formData.url" :disabled="true"></Bsipv4>
        </Form-item>
        <Form-item label="端口" prop="port">
          <Input v-model="formData.port" :maxlength=5></Input>
        </Form-item>
  ...
  data() {
    return {
      ...
      ruleDeviceForm: {
        ...
        port: [{ required: true, validator: deviceValidate.dePort, trigger: 'change', maxlength: 5 }],
