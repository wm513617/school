          ...
          <div class="res-add-model">
            ...
            <div class="res-model-tree" v-if="protoModel.isShow">
              <!-- <bs-scroll ref="scrollerThree" style="width:100%;"> -->
                <div style="width:100%;">
                <TreeBox  :searchToggle="true" :searchType="0" :resourceToggle="true" :iconToggle="false" :checkBox="true"  :currentNodeKey="cameraCurrentNodeKey"
                    :scroll="true" :equipmentToggle="true" :btnGroup="false" :orgType="11" :resType="[0]" :equType="[0]"   @on-expand="expandThree" @clickData="bindProtoClick" @checksData="checksData"></TreeBox>
              <!-- </bs-scroll> -->
              </div>
            </div>
          </div>
          ...
          <div class="res-add-model">
            ...
            <div class="res-model-tree" v-if="protoModel.isShow">
              ...
              <!-- <bs-scroll ref="scroller" style="width:100%;"> -->
                <TreeBox  :searchToggle="true" :searchType="0" :resourceToggle="true" :iconToggle="false" :checkBox="true"  :currentNodeKey="cameraCurrentNodeKey"
                    :scroll="true" :btnGroup="false" :orgType="11" :resType="[0]"  @clickData="bindProtoClick" @checksData="checksData"></TreeBox>
              <!-- </bs-scroll> -->
              </div>
            </div>
          </div>
      ...
      resColumns: [
        ...
        [
          ...
          },
          {
            ...
            render: (h, params) => {
              let text = ''
              if (params.row.guardData) {
                text = params.row.guardData.name
              } else {
                text = '...'
              }
              return h('span', text)
  ...
  methods: {
    ...
    addModalSave() {
      if (this.resActiveTab == 0) {
        ...
        if (!this.peopleCardBindParam.cameraData) {
          this.errorMsg('请选择资源')
          return
        }
    ...
    selectRows(rows) {
      console.log(rows)
      this.selectedCarmeID = []
      this.selectRes = rows
      this.selectRes.forEach(item => {
        if (item.cameraData.length > 0 && item.cameraData) {
          this.selectedCarmeID.push(item.cameraData[0]._id)
        }
      })
    },
    // 服务器类型
    ...
    // 绑定相机
    resBindPhoto() {
      this.cameraCurrentNodeKey = this.selectedCarmeID[0]
      this.protoModel.isShow = true
      if (this.selectRes.length === 1 && this.selectRes[0].cameraData.length > 0) {
    ...
    checksData(event) {
      ...
      console.log(event)
      // this.bindProtoParam._id = this.selectRes[0]._id;
      if (event.length === 2 && event[1].tierType === 'res') {
        this.bindProtoParam.cameraData.push(event[1]._id) // 如果接收到的值为资源的话，
      } else if (event.length === 1 && event[0].tierType === 'res') {
        this.bindProtoParam.cameraData.push(event[0]._id)
      }
      if (event.length === 2 && event[1].tierType === 'res') {
        this.peopleCardBindParam.cameraData.push(event[1]._id)
      } else if (event.length === 1 && event[0].tierType === 'res') {
        this.peopleCardBindParam.cameraData.push(event[0]._id)
      }
    },
    /// ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////// 人脸识别
    // 获取人脸识别列表
