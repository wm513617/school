...
export default {
  ...
  computed: {
    ...
    ...mapState({
      ...
      fireAlarmStateList: ({ alarmThreeD }) => alarmThreeD.fireAlarmList,
    ...
    fireAlarmData() {
      return this.fireAlarmList.slice(0, 21)
    },
    fireAlarmList() {
  ...
  watch: {
    ...
    fireAlarmStateList(newVal, oldVal) {
      if (newVal.length <= 20 && oldVal >= 20) {
        ...
        this.getMapAlarmList({type: 'fireAlarmList', data})
      }
    },
    isRecAlarm(newVal) {
      // 是否开启报警
  ...
  methods: {
    ...
    ...mapMutations(['CHANGE_SHOWVIDEO', 'SAVE_ITEM_ALARM_INPUT', 'SAVE_FIRE_ALARM_LIST', 'GET_ALARM_MODAL_TYPE', 'SET_ALARM_NEW_ONE', 'SPLICE_MAP_ALARM_LIST', 'UNSHIFT_MAP_ALARM_LIST', 'SET_ACTIVE_ALARM_INFO_3D']),
    expand() {
        ...
        this.$Modal.confirm({
          ...
          onOk: () => {
            ...
            alarmArr.push({
              ...
              devIp: val.devIp,
              devPort: val.devPort,
              channel: val.channel || val.chanId,
      ...
      if (data.alarmInfo.point3D) {
        ...
        if ((data.alarmInfo.eventType === 'fireAlarm' || data.alarmInfo.eventType === 'fireFailure') && data.alarmInfo.level >= this.filterLevel.fireControlLevel) {
          ...
          this.getMapAlarmPower({resId: infoData.channelId, type: '2'}).then(res => {
            ...
            this.UNSHIFT_MAP_ALARM_LIST({type: 'fireAlarmList', data: infoData})
    ...
    mapCleanFriePoint(index) {
      let mapData = this.fireAlarmList[index]
      if (mapData && mapData.point3D) {
        let obj = {
        ...
        try {
          // 报警列表清除该项
          this.SPLICE_MAP_ALARM_LIST({type: 'fireAlarmList', index})
