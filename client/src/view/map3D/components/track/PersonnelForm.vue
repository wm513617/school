...
import mapControl from '../../baseMap/mapControl.js'
let bodyEle
export default {
  name: 'PersonnelForm',
  mixins: [mapControl],
  ...
  methods: {
    ...mapActions([
      ...
      'setTrackCoordinates3D', // 设置2D轨迹坐标数组
      'setTrackCoMap',
      'setFaceTrackCoMap'
    ]),
    ...mapActions('map2DApplyIX', [
      'getTrackCond', // 根据条件查询人像轨迹数据
      'setFaceHistoryTrack3DNodes' // 设置人像历史轨迹节点数据
    ]),
    changeCamera() {
      const selectTree = this.$refs.devTree ? this.$refs.devTree.getSelectedDeepNameIds() : {}
    ...
    searchTrack() { // 确认查询
      ...
      this.getTrackCond(condition).then(res => {
        if (res && res.length > 0) {
          ...
          this.setShowTrackModal(false) // 关闭轨迹查询弹框
          this.setShowDrawTrack(true) // 显示绘制轨迹
          this.resetForm()
          // this.setFaceTrackCoMap(res) // 获取轨迹节点信息数据
          this.getTrackNodes(res)
        } else {
          this.setShowTrackModal(false) // 关闭轨迹查询弹框
      ...
      for (let index = trackList.length - 1; index >= 0; index--) {
        ...
        if (item.res && item.res.point) {
          ...
          if (coord && coord.length > 0) {
            ...
            trackNodeMap.set(attrObj, item.res.point.loc)
          }
        }
      }
    ...
    getTrackNodes(trackList) { // 获取轨迹节点信息数据
      ...
      for (const item of trackList) {
        ...
        if (pointId) {
          // index = index + 1
          // let attrObj = {index: index + 1, label: item.resName} // 构造点位数据
          // let cartesian3 = null
          let node = null
          if (trackNodeMap.has(pointId)) { // 节点map中已有设备信息
            ...
            if (point) {
              ...
              if (coord && coord.length > 0) {
                ...
                // attrCoMap.set(attrObj, cartesian3)
              }
            }
          }
    ...
    resetForm() {
      this.imageUrl = null
      this.veifaceCond.startTime = new Date(new Date().setHours(0, 0, 0, 0))
      this.veifaceCond.endTime = new Date()
      this.veifaceCond.similar = 75
    }
  },
  beforeDestroy() {
