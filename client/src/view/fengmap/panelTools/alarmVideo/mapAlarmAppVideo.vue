...
<template>
  <dragBoxs @close="close" :title="type === 'ManualAlarm' ? '手工报警' : '报警视频'" modelType="2D" :eventType="picType" :position="position">
    <div class="map-app-video-pre" style="width:815px;">
      <div v-if="type !== 'FaceAlarm'" class="mapAppInfoContent">
        ...
        <div class="infoleft" v-else>
          <alarmVideoMadel v-if="type !== 'ManualAlarm'" :stepTimeLine="0" ref="alarmVideo" :videoParam="actionList"></alarmVideoMadel>
        ...
        <div class="infoRight">
          ...
          <div class="infoRightInfo">
            ...
            <!-- 违章报警 -->
            <div v-else-if="type === 'IntelligentAlarm' && oneAlarmInData.eventType === 'vioRetrograde' || oneAlarmInData.eventType === 'vioPark' || oneAlarmInData.eventType === 'vioTurnLeft' || oneAlarmInData.eventType === 'vioTurnRight'">
              <div class="infoDetail">
                <div class="infoLabel">报警类型</div>
                <div class="infoValue">{{oneAlarmInData.eventTypeName}}</div>
              </div>
      ...
      <div v-else class="face-info">
        ...
        <div style="display:flex;" v-show="faceTab === '1'">
          <div class="face-info-left">
            ...
            <div class="face-img">
              <div style="width: 100%;height:24px;position: absolute; left: 0; top: 0; background-color:rgba(102, 102, 102, 0.3);"></div>
              ...
              <i class="font-common icon iconfont icon-guijichaxun1" title="轨迹追踪" @click="turnToTrail(alarmData.faceImage)"></i>
            </div>
            <div class="face-info-item">
        ...
        <div class="face-bottom-btn">
          <Button type="primary" style="margin-right:20px;"  :class="{'trail': isOpenTrail}" @click="changeTrail">实时追踪</Button>
          <Button type="primary"  style="margin-right:20px;" @click="cleanAlarm">清除报警</Button>
          <Button type="primary" @click="confirmAlarm">确认报警</Button>
          <!-- <i class="font-common icon iconfont icon-guijichaxun1" :class="{'trail': isOpenTrail}" style="cursor: pointer;vertical-align:middle;margin-left:24px;" title="轨迹追踪" @click="changeTrail"></i> -->
        </div>
      </div>
      <!-- 使用以下弹框避免视频插件遮挡弹框 -->
...
export default {
  ...
  data() {
    return {
      ...
      faceTab: '1',
      backParam: {},
      isOpenTrail: false // 针对 人像布控报警 是否开启轨迹追踪
    }
  },
  watch: {
    alarmData: {
      handler(newVal, oldVal) {
        if (this.type === 'FaceAlarm') {
          this.isOpenTrail = newVal.isOpenTrail || false
          this.backParam = {
            devIp: newVal.resIp,
  ...
  computed: {
    ...mapState({
      intelligentAlarmList: ({ mapAlarm }) => mapAlarm.intelligentAlarmList,
      selectedFaceAlarmArr: ({ mapAlarm }) => mapAlarm.selectedFaceAlarmArr
    }),
    ...mapGetters('mapAlarm', ['carType', 'carDirect']),
  ...
  methods: {
    ...mapMutations('mapAlarm', ['SAVE_NORMAL_ALARM_LIST', 'SAVE_FIRE_ALARM_LIST', 'SAVE_ALARM_HELP_LIST', 'SAVE_VIDEO_ALARM_LIST', 'SAVE_INTELLIGENT_ALARM_LIST', 'SAVE_ISMODAL_SURE_OR_CLEAN', 'CHANGE_FACE_TRAIL_STATUS', 'CHANGE_FACEALARM_TRAIL']),
    ...
    mapCleanPoint(data) {
      if (data.point3D) {
        let obj = {
          ...
          id: data.channelId || data.chanId,
          bcode: data.point3D.building3ds ? data.point3D.building3ds.code : '',
          type: this.type !== 'FireAlarmList' ? 'commonAlarm' : 'fireAlarm'
        }
    ...
    /* 打开/关闭 轨迹追踪 */
    changeTrail() {
      this.isOpenTrail = !this.isOpenTrail
      // 人像轨迹处理
      const list = JSON.parse(JSON.stringify(this.intelligentAlarmList))
      for (let i = 0; i < list.length; i++) {
        if (list[i].alarmId === this.alarmData.alarmId) {
          let param = { index: i, context: this.$context2D }
      ...
      // 人像轨迹完成后再修改按钮状态
      this.CHANGE_FACE_TRAIL_STATUS({...this.alarmData, isOpenTrail: this.isOpenTrail})
    },
    /* 人像详情框 跳转至轨迹追踪 */
    turnToTrail(img) {
      this.$emit('turnToTrail', img)
      this.close()
    }
  },
  created() {
