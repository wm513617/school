...
export default {
  ...
  computed: {
    ...mapState({
      ...
      pointState: ({fengMapApplyInteractive}) => fengMapApplyInteractive.pointState,
  ...
  created() {
    ...
    const pointState = window.localStorage.getItem('fengMapApplayPointState')
    if (pointState && pointState !== 'undefined') {
      this.savePoint(pointState)
    } else {
      window.localStorage.setItem('fengMapApplayPointState', JSON.stringify(this.pointState))
    }
  },
  methods: {
    ...mapMutations('fengMapApplyInteractive', ['CHANGE_IS_EXPAND_LEFT_TREE']),
    ...mapMutations(['SET_SELECTED_ATTR']),
    ...mapActions('fengMapApplyInteractive', ['savePoint', 'changeFilter', 'changePoint', 'switchToolsPanel', 'changeToolsPanelToBoxChoose',
