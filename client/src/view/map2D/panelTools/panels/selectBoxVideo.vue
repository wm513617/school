<template>
<Dragx title="选框镜头" @close="close" eventType="video">
  <div style="width:737px;" id='selectBoxVideo'>
    <div class="select-video-body" v-if="!showDownload">
      <bs-video :pluginCOM="plugin" :count="5" :styles="styles" @dblclick="switchStyle" @update:state="updateState" ref="bsvideo">
      </bs-video>
    </div>
    ...
    <Modal id="quickTowall" v-model="showTowall" title="快速上墙" :mask-closable="false" :transition-names="[]">
      ...
      <div slot="footer" style="position:relative">
        <Button @click="showTowall=false">关闭</Button>
      </div>
    </Modal>
  </div>
</Dragx>
...
export default {
  ...
  methods: {
    ...
    openModal(val) {
      this.videoList = this.$lodash.cloneDeep(this.pointList)
      this.modalShow = true
    },
    // 预览
    open() {
