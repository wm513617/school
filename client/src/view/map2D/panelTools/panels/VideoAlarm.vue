...
export default {
  ...
  computed: {
    ...mapState({
      ...
      isAlarmVideo: ({ map2DApplyIX }) => map2DApplyIX.filterState.isAlarmVideo,
        ...
        this.$Modal.confirm({
          ...
          onOk: () => {
            ...
            this.alarmToBeSure(alarmArr).then(res => {
              ...
              let index = this.videoAlarmList.findIndex(item => {
                return item.alarmId === val.alarmId
              })
              index !== -1 && this.SPLICE_MAP2D_ALARM_LIST({type: 'videoAlarmList', index})
              setTimeout(() => {
                this.$Modal.remove()
              }, 0)
            }).catch(() => {
