...
export default {
  ...
  computed: {
    ...
    ...mapGetters('mapAlarm', ['alarmTypeList', 'alarmTypeData', 'carType', 'carDirect']),
    filterAlarmList() {
      let filterList = []
  ...
  methods: {
    ...
    ...mapMutations('mapAlarm', ['CHANGE_SHOWVIDEO', 'SAVE_ITEM_ALARM_INPUT', 'SAVE_INTELLIGENT_ALARM_LIST', 'GET_ALARM_MODAL_TYPE', 'SET_ALARM_NEW_ONE', 'SET_MAP2D_ALARM_LIST', 'SPLICE_MAP2D_ALARM_LIST', 'SET_ACTIVE_ALARM_INFO', 'CHANGE_FACEALARM_TRAIL', 'SAVE_ALARM_CHANGE']),
    /* 改变 折叠状态 */
        ...
        this.$Modal.confirm({
          ...
          onOk: () => {
            ...
            this.alarmToBeSure(alarmArr).then(res => {
              // 报警列表清除该项
              const index = this.alarmInputList.findIndex(item => {
