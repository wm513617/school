<template>
  <div id="time-line" @mouseleave="mu">
        ...
        <div>
          <div class="color-lines" v-for="(option, i) in recordOptions" :key="i" :class="{'active': activedId === option.resId }" @click="lineClick($event,option.resId)" @mouseleave="leaveLine">
            ...
            <div class="color-line" :class="{'no-content': !(option.timedOptions.length || option.eventOptions.length)}" @mousemove="lineHover($event, i)">
              ...
              <!-- 案件录像 选择条 -->
              <div v-for="(item, index) in option.eventMark" :key="item.id" class="case-line" :class="{'active': activedId === option.resId && index === caseIndex }" @click.stop="caseDateClick(index, option.resId)" :style="{width: item.width + 'px', left: item.position + 'px'}"></div>
            </div>
            <!-- 剪切选择条 浮现文字 -->
    ...
    <!-- 录像标识 -->
    <div class="signs">
      <div class="case-video video">
        <span class="text">案件录像</span>
...
export default {
  ...
  computed: {
    ...
    ...mapState({
      ...
      activedId: ({ playback }) => playback.activedId, // 选中项的资源id
    ...
    recordOptions() {
      return this.recordInfo
        .filter(item => item)
        .map(({ eventVideo, timedVideo, manualVideo, label, resId, tags, tagTime }) => {
          return {
            // 事件录像
            ...
            // 事件标记
            eventMark: (tagTime || []).map(({ startTime, endTime, tagName }, index) => ({
              ...this.getPosition(startTime * 1e3, endTime * 1e3),
              id: 'e' + resId + '_' + index
            }))
          }
        })
  ...
  methods: {
    ...mapMutations(['CHANGE_ACTIVEID', 'SET_CASE_EDIT', 'SET_CASE_INDEX']),
    // 点击【事件录像】
    caseDateClick(index, id) {
      if (!this.activedId || id !== this.activedId) {
    ...
    mu(e) {
      ...
      if (this.recordInfo.length <= 0) { return }
      this.canMove = false
      if (this.isMoved) {
...
.signs .case-video span.color {
  background-color: #862feb;
}
