<template>
  <div class="tv-body" style="flex:1">
    <div>
      <div class="tabs" :class="{'active-tab': tabIndex===index}" :title='item.name' v-for="(item, index) in moreWall" :key="index" @click="changeTabIndex(index)">
        {{item.name}}
      </div>
      <div v-if="moreWall.length" class="tabs move" :class="{'active-tab': tabIndex === moreWall.length}" @click="changeTabIndex(-1)">
        +
      </div>
      <div v-if="moreWall.length" class="tabs move" :class="{'active-tab': delIndex === (moreWall.length + 1)}" style="left:-8px;" @click="deleteTabIndex">
        -
      </div>
    </div>
...
<script>
import AddTVWall from './tvwallEdit/AddTVWall'
import TVwallTab from './tvwallEdit/TVwallTab'
