...
export default {
  ...
  methods: {
    ...
    ...mapActions(['getWarningOrgTree', 'getWarningNews', 'getSortData', 'getWarningTypeTree', 'confirmWarning', 'getAlarmHostPowers', 'getAlarmDealSetList', 'getPrearranged', 'putBackupAlarmInfo', 'downloadInfo', 'inquireDownload']),
    resizeFun() {
    ...
    getTableData() {
      // 时间转换
      this.startTimestamp = Date.parse(new Date(this.formItem.startTime.replace(/-/g, '/'))) / 1000
      this.endTimestamp = Date.parse(new Date(this.formItem.endTime)) / 1000
      this.sureStartTime = Date.parse(new Date(this.formItem.confirmStartTime.replace(/-/g, '/'))) / 1000
      this.sureEndTime = Date.parse(new Date(this.formItem.confirmEndTime)) / 1000
      this.stopStartTime = Date.parse(new Date(this.formItem.resStartTime.replace(/-/g, '/'))) / 1000
      ...
      if (!this.sureTimeStuates && this.formItem.confirmStartTime === '' && this.formItem.confirmEndTime === '') {
        ...
        if (this.alarmTypeMeth === 'all') {
          let arry = []
          arry.push('alarmGT')
          for (let i in this.alarmDataTotal) {
            arry.push(i)
            this.alarmTypeMeth = arry.join(',')
            if (i === 'individualAlarm' || i === 'patrolAlarm' || i === 'manualAlarm' || i === 'focusAttention') {
              this.resType = 'noResTime'
            }
          }
        } else if (this.alarmTypeMeth === 'perimeterAlarm' || this.alarmTypeMeth === 'intrusionAlarm' || this.alarmTypeMeth === 'electricFence' || this.alarmTypeMeth === 'smoke' || this.alarmTypeMeth === 'temperature' || this.alarmTypeMeth === 'hydrant' || this.alarmTypeMeth === 'handNewspaper') {
          this.inquireType = 'fire'
        } else if (this.alarmTypeMeth === 'individualAlarm' || this.alarmTypeMeth === 'patrolAlarm' || this.alarmTypeMeth === 'manualAlarm' || this.alarmTypeMeth === 'focusAttention') {
          this.resType = 'noResTime'
        } else if (this.alarmTypeMeth === 'helpSeek') {
          this.alarmTypeMeth = 'alarmInput' + ',' + 'helpSeek'
        }
        const payload = {
          // 机构
          orgId: this.agencyId,
          ...
          type: this.inquireType ? this.inquireType : '',
          resType: this.resType ? this.resType : ''
        }
        // 报警确认开始时间和确认结束时间
    ...
    getArray(arry, name) {
      this.getWarningTypeTree().then(resp => {
        this.filter(resp.data, name)
      })
    },
    filter(list, val) {
      this.typeListValue = []
      ...
      let arryTwo = []
      for (let v in list) {
        if (v === val[0] && val.length === 2) {
          for (let i = 0; i < list[v].length; i++) {
            if (list[v][i].children) {
              list[v][i].children.forEach((item, index) => {
                arryOne.push(item.value)
              })
            } else {
              arryTwo.push(list[v][i].value)
            }
          }
        } else if (v === val[0] && val.length === 3) {
          for (let i = 0; i < list[v].length; i++) {
            if (list[v][i].children && list[v][i].children.length > 0 && list[v][i].children[0].value === val[2]) {
              for (let j in list[v][i].children) {
                arryOne.push(list[v][i].children[j].value)
              }
            }
          }
        }
      }
      this.typeListValue = arryOne.concat(arryTwo)
      this.typeListValue.forEach(item => {
        if (item === 'alarmZone') {
          this.typeListValue.push('alarmGT')
        } else if (item === 'focusAttention' || item === 'individualPolice') {
          this.resType = 'noResTime'
        }
      })
      this.alarmTypeMeth = this.typeListValue.join(',')
    },
    ...
    downLoadClickInfo() {
      this.inquireDownload(this.currentRow).then(res => {
        if (res.data.backup) {
        ...
        console.log('inquireDownload error: ', err)
      })
    }
  },
  mounted() {
  ...
  created() {
    ...
    this.getWarningTypeTree().then(resp => {
      this.alarmTypeList = this.warningTypeTreeData
    }).catch(err => {
  ...
  watch: {
    alarmTypeValue: {
      deep: true,
      handler(newValue) {
        if (newValue.length > 1 && (newValue[1] === 'intelligent' || newValue[1] === 'videoAlarm' || newValue[1] === 'normalAlarm' || newValue[1] === 'fireAlarm' || newValue[1] === 'individualPolice' || newValue[1] === 'alarmInput' || newValue[1] === 'alarmZone' || newValue[1] === 'monitoryPointAlarm' || newValue[1] === 'intelligentAlarm' || newValue[1] === 'violation')) {
          if (newValue[1] === 'individualPolice' || newValue[1] === 'videoAlarm') {
            this.resType = 'noResTime'
          } else {
            this.resType = ''
          }
          if (newValue.length === 3 && newValue[2] !== 'alarmInput' && newValue[2] !== 'alarmZone' && newValue[2] !== 'monitoryPointAlarm' && newValue[2] !== 'intelligentAlarm' && newValue[2] !== 'violation') {
            this.alarmTypeMeth = newValue[2]
          } else {
            this.typeListValue = []
            this.getArray(this.alarmTypeList, newValue)
          }
        } else {
          if (newValue[0] === 'alarmHelp') {
            this.alarmTypeMeth = 'askHelp' + ',' + 'acceptHelp' + ',' + 'endHelp'
            this.resType = 'noResTime'
          } else if (newValue.length === 2) {
            this.alarmTypeMeth = newValue[1]
            for (let i in this.inquireInfo) {
              if (i === newValue[1]) {
                this.inquireType = 'fire'
              }
            }
