...
import { GEOMETRYTYPE, RESOURCETYPE, MPSIGNKEY, MAPMODE, POINTSTATE } from '../../meta/common'
...
function getLineStringFeature(alarm, layerConfig, pointMode) {
  ...
  let feature = {
    ...
    attributes: {
      ...
      rType: layerConfig.rType, // 资源类型
      label: name,
      loc: (pointMode === MAPMODE.point2D) ? center : point.loc,
      isOuter: point.isouter
    },
    symbol: styleC
  }
 ...
 * @param {*} layerConfig 图层配置参数
 */
function transOneFeature(alarm, layerConfig, pointMode = MAPMODE.point2D) {
  let feature = null
  let { mapsign } = alarm
  let point = JSON.parse(JSON.stringify(alarm[pointMode]))
  if (point && point.loc) {
