...
const TIPS = '<p>左键双击、右键单击结束绘制</p><p>或按Esc键结束测量</p>'
...
export default class {
  ...
  regMsasureListeners() {
    ...
    document.addEventListener('keydown', this.handlePressKeydown) // 监听键盘按下
    _context.viewer.container.addEventListener('mouseenter', this.handleMouseEnter) // 监听鼠标移入地图
    _context.viewer.container.addEventListener('mouseleave', this.handleMouseLeave) // 监听鼠标移出地图
  }
  /**
   * 处理测量距离，距离变化
   */
  ...
  showMouseTips(position) {
    ...
    this.tooltip.showAt(position, TIPS)
  }
  /**
   * 关闭量算测量工具
   */
  ...
  /**
   * 处理假盘按下（使用箭头函数改变回调函数里的this的指向，使this指向该工具类）
   */
  ...
  /**
   * 处理鼠标移入地图（使用箭头函数改变回调函数里的this的指向，使this指向该工具类）
   */
  ...
  /**
   * 处理鼠标移出地图（使用箭头函数改变回调函数里的this的指向，使this指向该工具类）
   */
  ...
  relieveMeasureListeners() {
    ...
    _context.viewer.container.removeEventListener('mouseenter', this.handleMouseEnter) // 移除鼠标移入地图的监听
    _context.viewer.container.removeEventListener('mouseleave', this.handleMouseLeave) // 移除鼠标移出地图的监听
