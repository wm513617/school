...
export default {
  ...
  methods: {
    ...
    toggleDropdown(e) {
      if (!this.disabled) {
        if (
          (this.$refs.spans && e.target === this.$refs.spans[0]) ||
    ...
    isOptionSelected(option) {
      ...
      return typeof option === 'object'
        ? this.mutableValue[this.label] === option[this.label]
        : this.mutableValue === option
    },
    ...
    scrollToPosition(position) {
      if (this.$refs.dropdownMenu) {
        $(this.$refs.dropdownMenu).mCustomScrollbar('scrollTo', position, { scrollInertia: 0 })
      } // scrollInertia: 滚动持续时间
    },
