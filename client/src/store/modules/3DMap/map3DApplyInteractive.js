...
export default {
  ...
  state: {
    ...
    attrInfo: {
      // 点位信息
      type: 'AttrAlarm', // 枚举， 'AttrAlarm','AttrAlarmHelp','AttrBuilding', 'AttrFloor', 'AttrModel', 'AttrPatrol', 'AttrSinglePawn', 'AttrVideo'
      name: '',
      vendor: '',
      contacts: '',
      telContacts: 0,
    ...
    modalNumber: 0, // 3D地图弹窗个数
    isNavShow: true, // 导航栏展开/缩回
    // -------------------报警过滤状态----------------------
    filterState: {
      isCommonAlarm: { // 普通报警
        checked: true,
    ...
    isMeasurePanel3D: false, // 量算面板是否显示
    measureModeIndoor: { // 室内量算模式
      panelVisible: false, // 面板是否可见
      isMeasure: false, // 量算状态
      type: GeometryType.POLYLINE // 量算类型，默认是面
    },
    editVideoPointTag3D: 0,
    editAlarmPointTag3D: 0,
    faceHistoryTrack3DNodes: [] // 人像历史轨迹节点数组
  },
  getters: {
    // 点选状态
    ...
    isEmergency(state) {
      // 是否开启应急预案开关
      console.log(state.isEmergency, 'state.isEmergency2222')
    ...
    // 获取所有点位元素是否勾选状态
    getpointSHLists: state => state.pointSH,
    isCameraSpear(state) {
      // 点位-枪机
    ...
    isSingleHead(state) {
      // 点位-单兵头像
      return state.pointSH.isSingleHead
    },
    isShowName(state) {
      // 是否显示名称标签
    ...
    measureModeIndoor: state => { // 获取室内量算模式
      return state.measureModeIndoor
    },
    isMeasurePanel3D: state => { // 获取3D量算面板是否显示
      return state.isMeasurePanel3D
    },
    isMeasurePanelIndoor: (state, getters) => { // 获取室内量算面板是否显示
      return getters.measureModeIndoor.panelVisible
    },
    editVideoPointTag3D(state) {
      return state.editVideoPointTag3D
    },
    editAlarmPointTag3D(state) {
      return state.editAlarmPointTag3D
    },
    faceHistoryTrack3DNodes(state) { // 人像历史轨迹节点数组
      return state.faceHistoryTrack3DNodes
    }
  },
  mutations: {
    ...
    CHANGE_IS_NAV_SHOW(state, data) {
      state.isNavShow = data
    },
    CHANGE_ITEM_STATE(state, arr) {
      let result = state.filterState[arr[0]]
      arr[1]
    ...
    CHANGE_MEASURE_MODE_INDOOR(state, mode) { // 改变室内量算模式
      for (const key in mode) {
        if (state.measureModeIndoor.hasOwnProperty(key)) {
          state.measureModeIndoor[key] = mode[key]
        }
      }
    },
    CHANGE_IS_MEASURE_PANEL_3D(state, flag) { // 改变量算面板是否显示
      state.isMeasurePanel3D = flag
    },
    EDIT_VIDEO_POINT_TAG_3D(state, num) {
      state.editVideoPointTag3D = num
    },
    EDIT_ALARM_POINT_TAG_3D(state, num) {
      state.editAlarmPointTag3D = num
    },
    SET_FACE_HISTORY_TRACK_3D_NODES(state, arr) { // 设置人像历史轨迹节点数据
      state.faceHistoryTrack3DNodes = arr
    }
  },
  actions: {
    // ISPOINTCHOOSESTATUS
    ...
    // 改变报警过滤按钮状态
    changeAlarmFilterState({ commit }, arr) {
      commit('CHANGE_ITEM_STATE', arr)
    },
    changeAlarmFilterLeavel({ commit }, arr) {
      commit('CHANGE_FILTER_LEAVEL', arr)
    },
    // 改变室内量算模式
    changeMeasureModeIndoor({ commit }, mode) {
      commit('CHANGE_MEASURE_MODE_INDOOR', mode)
    },
    // 改变3D量算面板是否显示
    changeIsMeasurePanel3D({ commit }, flag) {
      commit('CHANGE_IS_MEASURE_PANEL_3D', flag)
    },
    setFaceHistoryTrack3DNodes({ commit }, arr) { // 设置人像历史轨迹节点数据
      commit('SET_FACE_HISTORY_TRACK_3D_NODES', arr)
    }
  }
}
  ...
  switch (params.type) {
    ...
    case 'floor': // 楼层
      return {
        type: 'AttrFloor',
        name: params.data.name || '',
        bid: params.data.bid._id || '',
        bName: params.data.bid.name || '',
        contactsInfo: params.data.pid ? [{
          contacts: params.data.pid.name || '',
          telContacts: params.data.pid.mobile || ''
        }] : [],
        id: params.data._id || '',
        storey: []
      }
