<template>
  <div class="video-plugin" :class="[isFullScreen ? 'fs' : '']">
    ...
    <div class="progress-bar-parent" v-if="configuration.progressBar">
    ...
    <div class="video-bottom" v-if="isButton">
      <div class="left">
        ...
        <i class="icon iconfont icon-stop" :class="[!selectedObj.playStatus ? 'disabled' : '']" @click="stop(checkedPane, true)" title="停止" v-if="!buttos.includes('stop')"></i>
        <i class="icon iconfont icon-stop-all" :class="[stopStatus? 'disabled' : '']" @click="stop()" title="停止全部" v-if="!buttos.includes('stopAll')"></i>
        <i class="icon iconfont icon-xiazai" :class="[selectedObj.dumpHandle !== '' ? 'disabled' : '']" title="下载" @click="openDownload" v-if="!buttos.includes('xiazai')"></i>
        <i class="icon iconfont icon-screenshot" v-if="!buttos.includes('screenshot')" title="截图" @click="getCapture"></i>
        <i class="icon iconfont icon-frame-forward" v-if="!buttos.includes('frameByFrame')" title="逐帧进" @click="clickByFrame"></i>
        <i class="icon iconfont" :class="[!selectedObj.lookDown? 'icon-nk-close': 'icon-niaokan']" title="鸟瞰" @click="clickLookDown"></i>
        ...
        <div class="dp-con showmodeShow" @mouseenter.stop="showSpeed=true" @mouseleave="showSpeed=false" v-if="!buttos.includes('playbackSpeed')">
          <i class="icon iconfont" :class="[!selectedObj.isPlay?'disabled':'',selectedObj.pitchSpeed.icon]" title="播放速度"></i>
          <ul v-show="showSpeed&&selectedObj.isPlay" class="showmode-list" style="z-index:999">
            <i></i>
            <li v-for="(speed, i) in speedList" :key='i' @click="clickSpeed(speed)" :class="{active: selectedObj.pitchSpeed.icon===speed.icon}">{{speed.label}}</li>
          </ul>
        </div>
        <div class="dp-con showmodeShow" @mouseenter="showmodeShow=true" @mouseleave="showmodeShow=false" v-if="appendButtos.includes('showmodeShow')">
        ...
        <i class="icon iconfont icon-calendar" v-if="appendButtos.includes('calendar')" title="日历" @click="clickCalendar"></i>
      </div>
      <div class="right" v-if="rightControl">
    ...
    <div class="solt-reserved-control" v-if="reservedControl">
      <slot name="reserved-control"></slot>
    </div>
      ...
      <div class="one-stage">
        ...
        <div class="timeline-stage-bottom">
          ...
          <div class="timeline-video-segments">
            <div class="segments" @mousedown.stop="()=>{}" @mouseup.stop="()=>{}">
              <div class="segments-son" v-for="(item, index) in singleStage" :index="item.index" :key="index" :style="{left: item.left, width: item.width, backgroundColor: item.bgc, zIndex: item.zIndex}"></div>
            </div>
          </div>
        </div>
      </div>
...
import { openPlaybackList, stopRecordPlay, downloadVideoSegments, checkVideoList, videoFilter, setRecordPlayerRate, setRecordPlayerMode } from './playback.js'
...
export default {
  ...
  props: {
    ...
    configuration: {
      ...
      default: () => {
        return {
          ...
          // buttos: ['stopAll', 'onTheWall'] // 数组有的按钮都不显示  onTheWall: 上墙，stopAll：全部关闭, showmodeShow:多画面, stop: 停止按钮， xiazai：下载按钮， screenshot：截图按钮， screen：全屏， volume：音量, playbackSpeed：倍速调节, frameByFrame: 逐帧进
          buttos: []
        }
      }
    },
    appendButtos: {
      type: Array,
      default: () => []
    }, // 新增的按钮， 数组有的按钮显示   showmodeShow 画面分割, calendar 日历
    windowAdapter: {
      // 窗口适配参数
      type: Array,
      default: () => []
  ...
  computed: {
    ...
    singleVideoHeight() {
      // 要减去的按钮及操作栏高度
      let height = 0
      if (this.reservedControl) {
        // 额外要减去的宽度
        height = 56
      }
      if (this.configuration.timeline) {
        if (this.configuration.isMultistage) {
        // 多路设备时间轴 + 按钮
          return 170 + 56 + height
        }
        // 单路设备时间轴 + 按钮
        return 100 + 56 + height
      } else if (this.configuration.progressBar) {
        // 进度条 + 按钮
        return 24 + 56 + height
      }
      // 按钮
  ...
  data() {
    return {
      ...
      singleStage: [
        // { left: '20%', width: '50px', bgc: '#990033', zIndex: 1 },
        // { left: '20%', width: '20px', bgc: '#FF3366', zIndex: 9 },
        // { left: '50%', width: '50px', bgc: '#CC3333', zIndex: 1 },
        // { left: '80%', width: '50px', bgc: '#FF0000', zIndex: 1 }
      ], // 单个时间轴上标记多段录像段数组
      colorArr: [
        '#990033',
        '#FF3366',
  ...
  mounted() {
    ...
    if (this.configuration.timeline) {
      document.querySelector('.segments').addEventListener('click', (e) => {
        this.controlSignBackground(e.target)
        e.stopPropagation()
      })
    }
  },
  methods: {
    ...mapActions(['getVideoConf', 'getMonitorList', 'getTVList', 'getCameraPower']),
    kk() {
      let arr = [
        // {
        //   name: '20.36',
        ...
        // },
        {
          ...
          eventType: ['all'],
          // startTime: 1563897600,1563972653
          startTime: 1575216000,
          streamType: 'main',
          // occurrenceTime: 1565150400,
          resId: '5dea182cb915d23760ca9a24'
    ...
    stop(index, playStatus = false) {
      try {
        ...
        if (index !== undefined) {
          ...
          this.updateStatus(index, Object.assign(originalState, { isPlay: false, playStatus: playStatus }))
        ...
        for (let i = 0; i < this.videoStatusArr.length; i++) {
          ...
          let originalState = JSON.parse(JSON.stringify(this.originalState))
          this.updateStatus(i, Object.assign(originalState, { isPlay: false, playStatus: playStatus, result: {}, isSyncPlay: false }))
        }
        window.clearInterval(this.progressTimer)
        this.progress = 0
      } catch (err) {
    ...
    async openPlayback(data, isQuery = true, womod) {
      ...
      for (let i = 0; i < playbackNumber; i++) {
        if (!data[i].appName) { // 如果有appName字段，则不判断权限，否则根据resId判断权限
    ...
    async anewOpenPlayback(newTime, isQuery = false, paneMark) {
      ...
      this.clickSpeed(this.videoStatusArr[this.checkedPane].pitchSpeed) // 同步倍速
    ...
    initSingleStage() {
      ...
      for (let index = 0; index < this.videoStatusArr.length; index++) {
        ...
        if (item.playStatus && Object.keys(item.result).length !== 0) {
          ...
          item.eventList.forEach((video, index) => {
            resultArr.push({
              ...
              index,
              original: {
                startTime: video.evtTblInfo.startTime,
                endTime: video.evtTblInfo.endTime,
                bgc: '#32e184',
                highlightBgc: '#32e184'
              }
            })
      ...
      if (this.signVideoSegment.length !== 0) {
        // 标记段的数组不为空
        this.signVideoSegment.forEach(video => {
          resultArr.push({
            left: `${((video.startTime - timeScaleStaTime) / sumTime) * 100}%`,
            width: `${(video.endTime - video.startTime) * secondPx}px`,
            bgc: video.bgc,
            zIndex: 9,
    ...
    // 录像段标记函数
    appendSingleStage(videoSegments) {
      // videoSegments: [{startTime: '1565020800', endTime: '1565020800', bgc: '#ccc', highlightBgc: '#fff'}] // 参数格式, 传空数组 就是去掉之前加的标记段
      this.signVideoSegment = videoSegments || []
      this.initSingleStage()
    },
    // 录像段操作函数
    controlSignBackground(e) {
      let children = document.querySelector('.segments').children
      for (const iterator of children) {
        let index = iterator.getAttribute('index')
        if (iterator !== e) {
          // 去掉高亮
          iterator.style.backgroundColor = this.singleStage[index].bgc
          iterator.style.zIndex = this.singleStage[index].zIndex
        } else {
          // 加上高亮
          iterator.style.backgroundColor = this.singleStage[index].original.highlightBgc
          iterator.style.zIndex = 99 // 层级高些
          this.$emit('clickSignleStage', this.singleStage[index].original || this.singleStage[index].bgc) // 点击标记段事件抛出
        }
      }
    }
    // 单个时间轴 end
