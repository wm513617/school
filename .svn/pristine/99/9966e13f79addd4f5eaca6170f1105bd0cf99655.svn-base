...
export default {
  ...
  computed: {
    ...mapState({
      normalAlarmList: ({ fengMapAlarm }) => fengMapAlarm.normalAlarmList, // 普通报警列表
      fireAlarmList: ({ fengMapAlarm }) => fengMapAlarm.fireAlarmList, // 消防报警列表
      alarmHelpList: ({ fengMapAlarm }) => fengMapAlarm.alarmHelpList, // 报警求助列表
      videoAlarmList: ({ fengMapAlarm }) => fengMapAlarm.videoAlarmList, // 视频报警列表
      intelligentAlarmList: ({ fengMapAlarm }) => fengMapAlarm.intelligentAlarmList, // 智能报警列表
      activeAlarmTag: ({ fengMapAlarm }) => fengMapAlarm.activeAlarmTag,
      activeAlarmInfo: ({ fengMapAlarm }) => fengMapAlarm.activeAlarmInfo,
      mapConfigArr: ({ mapIndex }) => mapIndex.mapConfigArr,
      filterState: ({ fengMapApplyInteractive }) => fengMapApplyInteractive.filterState,
      filterLevel: ({ fengMapApplyInteractive }) => fengMapApplyInteractive.filterLevel,
      videoAlarmIdInfo: ({ fengMapAlarm }) => fengMapAlarm.videoAlarmIdInfo,
      sosAlarmIdInfo: ({ fengMapAlarm }) => fengMapAlarm.sosAlarmIdInfo,
      singleTimeoutList: ({ fengMapAlarm }) => fengMapAlarm.singleTimeoutList,
      attrInfo: ({ mapPoint }) => mapPoint.mapResourceAttributes,
      alarmTabData: ({ fengMapAlarm }) => fengMapAlarm.alarmTabData,
    ...
    ...mapGetters('fengMapApplyInteractive', ['isEmergency', 'isAutoPosition', 'isFaceSnap', 'isInfoPanelFixed', 'isPlatformTrack']),
  ...
  watch: {
    ...
    facePointList: { // 人脸报警数据
      handler(arr) {
        if (arr && arr.length) { // 有人像报警数据时
          if (this.isFaceSnap && !this.isFaceAlarmTrack && !this.fmapShowTrackModal && !this.fmapShowDrawTrack) { // 不显示轨迹查询弹框 、不显示绘制轨迹 且 不显示绘制轨迹时
            this.updateFaceSnaps(arr) // 更新人像抓拍图片
          }
        } else { // 无人像报警数据时，清空人脸头像数组
          this.deleteFaceHeadDatas('faceSnap') // 删除人脸头像数组
        }
      },
      deep: true
    },
    ...
    isFaceAlarmTrack(flag) { // 人像布控轨迹状态
      if (!flag && this.isFaceSnap && !this.fmapShowDrawTrack && !this.fmapShowTrackModal) { // 不显示人像布控轨迹且显示抓拍图片时
    ...
    fmapShowTrackModal(flag) { // 是否显示轨迹查询弹框状态
      if (flag) { // 显示绘制轨迹时
        ...
        !this.fmapShowDrawTrack && this.isFaceSnap && this.updateFaceSnaps(this.facePointList) // 不显示绘制轨迹 且显示人像布控抓拍图片时，更新人像抓拍图片
      }
    },
    fmapShowDrawTrack(flag) { // 是否显示绘制轨迹状态
      if (flag) { // 显示绘制轨迹时
        this.deleteFaceHeadDatas('faceSnap') // 删除人脸头像数组
      } else { // 不显示绘制轨迹状态时
        !this.fmapShowTrackModal && this.isFaceSnap && this.updateFaceSnaps(this.facePointList) // 不显示轨迹查询弹框 且 显示人像布控抓拍图片时，更新人像抓拍图片
      }
    }
  },
  methods: {
    ...
    ...mapMutations('fengMapAlarm', ['SET_SIX_TYPE_ALARM_POINT_LIST', 'SET_ALLVIDEO_ALARM_POINT_LIST', 'SET_SOS_ALARM_POINT_LIST', 'SET_SIX_TYPE_ALARM_BUILD_LIST', 'SET_ALLVIDEO_ALARM_BUILD_LIST', 'SET_SOS_ALARM_BUILD_LIST', 'PUSH_VIDEO_ALARM_ARR', 'SET_VIDEO_ALARM_ARR', 'PUSH_SOS_ALARM_ARR', 'SET_SOS_ALARM_ARR', 'SET_SINGLE_TIMEOUT_ALARM_List', 'UPDATE_ALARMTAB_DATA' ]),
    ...mapMutations(['SET_MAP_RESOURCE_ATTRIBUTES', 'CHANGE_SINGLE_TIMEOUT_ALARM_STATUS', 'SET_SHOW_MAP_RESOURCE_ATTRIBUTES']),
    ...mapMutations('fengMapApplyInteractive', ['SAVE_FILTERLEVEL', 'SAVE_FILTERSTATE']),
    ...mapActions('fengMapApplyInteractive', ['switchEmergency', 'switchAutoPosition']),
    ...mapActions('fengMapFace', ['setFaceHeadDatas', 'deleteFaceHeadDatas', 'clearFaceHeadDatas']),
    ...
    updateFaceSnaps(arr) { // 更新人像图片显示
      ...
      for (const item of arr) { // 遍历人脸报警列表数据
        if (item.point) { // 有点位位置信息时
          ...
          let coordinates = [12946801.463598568, 4861604.337475137]
          if (coordinates && coordinates.length) { // 坐标点位有效时
            const faceType = this.isFaceAlarmTrack ? 'alarmTrack' : 'faceSnap'
            if (faceSnapMap.has(pointId)) { // 抓拍图片存在时，更新抓拍图片为最近人像布控报警的抓拍图片
              ...
              if (faceSnap.timestamp < item.timestamp) { // 点位抓拍图片不是最近时间时，更新点位抓拍图片
                faceSnap.id = faceType + item.timestamp + '_' + pointId // 更新抓拍图片数据标识
                ...
                faceSnap.faceType = faceType
              }
            } else { // 抓拍图片不存在时
              faceSnap = {
                id: faceType + item.timestamp + '_' + pointId,
                userId: item.userId,
                borderColor: this.defaultFaceSnapBorderColor,
                userImage: item.faceImage,
                coordinates: coordinates,
                userName: item.userName,
                pointId: pointId,
                pointName: item.resName,
                faceType: faceType
              }
            }
          }
      ...
      this.setFaceHeadDatas(faceSnaps)// 设置人脸头像数组
    },
  ...
  created() {
    // 报警过滤
    let filterState = window.localStorage.getItem('fengMapAlarmFilterState')
    if (!filterState) {
      window.localStorage.setItem('fengMapAlarmFilterState', JSON.stringify(this.filterState))
    } else {
      this.SAVE_FILTERSTATE(JSON.parse(filterState))
    }
    let filterLevel = window.localStorage.getItem('fengMapAlarmFilterLevel')
    if (!filterLevel) {
      window.localStorage.setItem('fengMapAlarmFilterLevel', JSON.stringify(this.filterLevel))
    } else {
      this.SAVE_FILTERLEVEL(JSON.parse(filterLevel))
    }
    // 自动定位 应急预案
    let switchStatus = window.localStorage.getItem('fengMapAlarmswitch')
    if (!switchStatus) {
      switchStatus = {
        isAutoPosition: this.isAutoPosition,
        isEmergency: this.isEmergency
      }
      window.localStorage.setItem('fengMapAlarmswitch', JSON.stringify(switchStatus))
    } else {
