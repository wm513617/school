  ...
  <section class="tools" :style="{height: !toolVisible? 0 : '100%'}">
    ...
    <section class="tool-area" :class="{'hidden': !toolVisible}">
      <ul class="tool-list" :class="{'hidden': !toolVisible}">
        ...
        <li title="轨迹刻画" v-if="isfmOuter && fmapLoadComplete">
          <i
            @click="changeTrackStatus"
            class="icon iconfont icon-guijichaxun1"
            :class="{'active': fmapShowDrawTrack || showDrawTrack2D}"
            type="ios-shuffle-strong"
            :style="{'cursor': isPlatformTrack ? 'not-allowed' : 'pointer'}"
          ></i>
        </li>
        <li title="量算" v-if="fmapLoadComplete">
          <i @click="changeMeasureStatus"
            ...
            :style="{'cursor': isPlatformTrack ? 'not-allowed' : 'pointer'}"
          >
            <div v-if="showMeasurePanel" class="measure-box">
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      isOpenTrail: ({ fengMapApplyInteractive }) => fengMapApplyInteractive.isOpenTrail, // 人脸轨迹数据
      ...
      boxChooseType: ({ fengMapApplyInteractive }) => fengMapApplyInteractive.boxChooseType,
    ...
    ...mapGetters('fengMapDraw', ['isPointDrawActive', 'isCircleDrawActive', 'isLineDrawActive']),
    ...mapGetters('fengMap', ['boxChooseSecPanelStatus', 'isfmOuter', 'fmapShowDrawTrack', 'fmapShowTrackModal', 'showMeasurePanel']),
    alarmList() {
      let alarmLists = []
  ...
  watch: {
    ...
    isfmOuter(flag) {
      ...
      this.setMeasurePanelVisible(false) // 关闭量算工具
    },
    currentFloor: {
      // 当前楼层的数据
      handler(data) {
    ...
    isShowPTattr(val) {
      if (val) {
        ...
        this.setMeasurePanelVisible(false) // 关闭2D量算状态
        this.SET_BOX_SEC_PANEL_STATUS(false)
      }
    },
    isSelectBoxPreview(val) {
      if (val) {
    ...
    showMeasurePanel(flag) {
      if (flag) {
        // 开启量算时
        this.boxChooseSecPanelStatus && this.SET_BOX_SEC_PANEL_STATUS(false) // 关闭框选面板
        this.changeIsPointChoose(false)
        this.closeToolsPanel() // 关闭其他面板
        if (this.fmapShowTrackModal || this.showDrawTrack2D) {
  ...
  methods: {
    ...mapActions('fengMapDraw', ['activeDrawFn', 'closeDrawFn']),
    ...
    ...mapActions('fengMapAlarm', ['getMapAlarmList', 'getMapAllAlarmList']),
    ...
    ...mapMutations('fengMapApplyInteractive', [
      'CHANGE_IS_OPEN_FIRST_TOOL_PANNEL',
      'CHANGE_IS_GRID_STATISTIC',
      'SET_PLATFORM_TRACK',
      'TRACK_INFO_TO_MAP_TAB'
    ]),
    ...mapMutations('fengMap', ['SET_BOX_SEC_PANEL_STATUS', 'SET_FM_OUTER', 'SET_FMAP_SHOW_DRAW_TRACK', 'SET_FMAP_SHOW_TRACK_MODAL']),
    ...mapActions('fengMap', ['setMeasurePanelVisible', 'changeMeasureType']),
    initToolTiper() {
    ...
    showSecPanel() {
      ...
      status = !status
      this.SET_BOX_SEC_PANEL_STATUS(status) // 切换框选面板
      console.log(this.toolsPanelActive)
      if (this.toolsPanelActive !== 'boxChoose') {
        this.openToolsPanel('')
        this.switchToolsPanel(false) // 是否显示二级面板
      }
      this.showMeasurePanel && this.setMeasurePanelVisible(false)
      /*
      */
    ...
    changeTrackStatus() {
      // 改变轨迹状态
      this.changeIsPointChoose(false)
      this.setMeasurePanelVisible(false) // 关闭量算
    ...
    controlTrackSearch() {
      // 控制轨迹查询显示
      if (this.fmapShowDrawTrack || this.showDrawTrack2D) {
        this.SET_FMAP_SHOW_TRACK_MODAL(false) // 设置不显示轨迹查询弹出框
        this.SET_FMAP_SHOW_DRAW_TRACK(false) // 设置不显示绘制轨迹
      } else if (!this.fmapShowTrackModal) {
        // 未打开轨迹查询弹出框时
        this.SET_FMAP_SHOW_TRACK_MODAL(true) // 设置显示轨迹查询弹出框
      }
    },
    openPanel(v) {
      if (this.isPlatformTrack) {
      ...
      this.changeIsPointChoose(false)
      this.chooseDraw = false
      this.setMeasurePanelVisible(false) // 关闭量算状态
      this.SET_BOX_SEC_PANEL_STATUS(false)
    ...
    boxChooseStatus(type) {
      this.activeDrawFn(type) // 激活对应框选类型
      // console.log(this.isPointDrawActive)
      /* this.changeIsPointChoose(false)
      // 涉及点选不清除预览资源
    ...
    changeMeasureStatus() {
      ...
      // 改变量算状态
      let status = !this.showMeasurePanel
      this.setMeasurePanelVisible(status)
    },
    measureDistance() {
      // 测距
      let type = GEOMETRYTYPE.POLYLINE
      this.changeMeasureType(type) // 改变2D量算模式
    },
    measureArea() {
      // 测面
  ...
  beforeDestroy() {
    ...
    this.closeToolsPanel()
    this.setMeasurePanelVisible(false)
    this.CHANGE_SHOWVIDEO(false)
    this.CHANGE_REPORT_POLICE(false)
