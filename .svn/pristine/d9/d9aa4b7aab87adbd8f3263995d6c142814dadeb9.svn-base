...
import layerConfig from '../meta/layer'
...
function addVedio(array) {
  ...
  if (array.length > 0) {
    array.forEach(element => {
      if (element.point) {
        let { _id, type, point, status, monitortype } = element
        let symbol = ipcUtil.getStorageVedioSymbolByIpcTypeAndState(monitortype, status) // 当前要添加的feature的样式
        let coods = point.loc.split(',')
        let newSymbol = JSON.parse(JSON.stringify(symbol))
        newSymbol.iconStyle.rotation = point.angle
        let feature = {
          geom: {
          ...
          attributes: {
            id: _id,
            type: layerConfig.video.name, // 类型
            rType: type, // 资源类型
...
function addSector(array) {
  ...
  if (array.length > 0) {
    array.forEach(element => {
      if (element.point) {
        let { _id, type, point, monitortype } = element
        let coods = point.loc.split(',')
        let attr = {
          id: _id,
          rType: type, // 资源类型
