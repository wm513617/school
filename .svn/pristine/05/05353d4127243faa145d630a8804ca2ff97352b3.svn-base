    ...
    resourceNode(node) {
      ...
      switch (this.resourceType) {
        ...
        case '4':
          ...
          break
        case '6':
          if ('pid' in node || 'type' in node || 'isPassRoot' in node) { // 选中资源节点
            this.copySet = state
            ...
            if ('isPassRoot' in node && !state) {
              // 选中的是人员机构下根节点
              this.propertySetStatus(false, 2)
            }
            this.resourceId = node._id
            let nodeArr = this.searchNode(node._id)
            if (nodeArr.length === 0) {
      ...
      switch (this.resourceType) {
        ...
        case '6':
          if (type === 0 || type === undefined) {
            this.trafficPropertySet.showSet = status
            this.trafficPropertySet.otherSet = status
            this.trafficPropertySet.orgSet = true
          } else if (type === 1) {
            this.trafficPropertySet.showSet = false
            this.trafficPropertySet.otherSet = true
            this.trafficPropertySet.orgSet = true
          } else {
            this.trafficPropertySet.showSet = true
            this.trafficPropertySet.otherSet = true
            this.trafficPropertySet.orgSet = false
          }
          break
      }
    },
        ...
        case '6':
          return this.trafficPropLists.map((item, index) => {
            switch (item.labelValue) {
              ...
              case 'delOrg':
              case 'updateOrg':
                item.isDisabled = this.trafficPropertySet.orgSet
                break
              default:
