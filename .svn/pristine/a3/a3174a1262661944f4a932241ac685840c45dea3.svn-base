        ...
        <div v-for="(items, indexs) in totalAlarmList" class="outer-layer" style="border-radius: 4px;" :key="indexs">
            ...
            <div style="float:left;width:76%;height:100%;cursor:pointer" @click="messageMain(items)">
              ...
              <p>
                <span style="width:42%"><i class="font-common icon iconfont icon-admin"></i>
                  <span>{{items.message.sender}}</span>
                </span>
              </p>
              <p>
                <span style="width:42%"><i class="font-common icon iconfont icon-Location"></i>{{items.message.position}}</span>
              </p>
              <p>
          ...
          <div class="single-alarm-li" v-if="items.alarmTypeToMap === 'singleAlarm' || items.alarmTypeToMap === 'generalMsg'">
            <div @click="singleAlarm(items)">
              ...
              <ul class="left-ul">
                  ...
                  <li title="用户名称"><i class="iconfont icon-admin default-size"></i> <span>{{items.realname || items.sender}}</span></li>
                  <!-- <li title="任务完成百分比"><i class="iconfont icon-jindu" style="font-size:13px;"></i> {{items.precentage || '0%'}}</li> -->
                  <li class="left-li" :title="(items.time)"><i class="icon iconfont icon-shijian" style="font-size:13px;"></i> <span>{{items.time || items.date}}</span> </li>
              </ul>
            </div>
            <div class="right-btn">
          ...
          <div v-if="items.alarmTypeToMap === 'talkAlarm'">
            <div style="cursor: pointer;float:left">
              ...
              <ul class="left-ul">
                  <li title="报警分类"><i class="icon iconfont icon-fenlei"></i> <span>{{items.type}}</span></li>
                  <li title="用户名称"><i class="iconfont icon-admin default-size"></i> <span>{{items.realname}}</span></li>
...
export default {
  ...
  computed: {
    ...
    ...mapState({
      isRelyModal: ({ patrolAlarm }) => patrolAlarm.isRelyModal,
      singleAlarmData: ({ patrolAlarm }) => patrolAlarm.singleAlarmData,
      alarmType: ({ patrolAlarm }) => patrolAlarm.alarmType,
      patrolShow: ({ patrolAlarm }) => patrolAlarm.patrolShow,
      isAlarmSingle: ({ map3DApplyIX }) => map3DApplyIX.filterState.isAlarmSingle,
      patrolId: ({ patrolAlarm }) => patrolAlarm.patrolId,
      singleId: ({ patrolAlarm }) => patrolAlarm.singleId,
      singleAlarmInDatabase: ({ patrolAlarm }) => patrolAlarm.singleAlarmInDatabase, // 从数据库中获取的报警数据
      modalStatus: ({ patrolAlarm }) => patrolAlarm.modalStatus
    }),
    totalAlarmList() {
      let filterList = []
  ...
  mounted() {},
  methods: {
    ...mapActions(['confirmAlarms', 'getPrearranged', 'nearestSingle', 'emergencyAction', 'confirmSingle', 'getMapAlarmList']),
    ...mapActions('map3DApplyIX', ['changeOpenAlarmPanel']),
    ...mapMutations(['GET_PATROL_TYPE', 'SAVE_PATROL_STUATES', 'SAVE_PATROL_DATA', 'SAVE_SINGLE_DATA', 'SAVE_ALARM_TYPE', 'SAVE_PATROL_RELY_STUATES', 'SET_ALARM_NEW_ONE', 'SET_SINGLE_TOTAL', 'SAVE_PATROL_ID', 'SET_ACTIVE_ALARM_INFO_3D', 'SAVE_TALK_DATA']),
    expand() {
    ...
    // 一键报警
    videoCall(obj) {
      console.log('巡更', this.modalStatus)
      if (this.modalStatus) {
        return
      }
     ...
     */
    messageMain(item) {
      // console.log('点击巡更数据', item)
      const obj = {...item}
      obj.type = 'patrolAlarm'
      this.SET_ACTIVE_ALARM_INFO_3D(obj)
      if (item.map3D) {
        if (!item.map3D.isouter) {
          let sid
          if (typeof (item.map3D.sid) === 'string') {
            sid = item.map3D.sid
          } else {
            sid = item.map3D.sid._id
          }
          this.focusOnFloorAlarm(sid)
        }
      } else if (item.message && item.message.point3D) {
        if (!item.message.point3D.isouter && item.message.point3D.sid) {
          this.focusOnFloorAlarm(item.message.point3D.sid)
        }
      }
    },
    singleAlarm(item) {
      const obj = {...item}
      obj.type = 'singleAlarm'
      this.SET_ACTIVE_ALARM_INFO_3D(obj)
      // console.log('单兵点击数据', item)
      const id = item.uniqueId || item._id || item.uid
