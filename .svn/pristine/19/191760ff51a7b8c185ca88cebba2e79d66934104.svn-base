...
export default {
  ...
  computed: {
    ...mapState({
      ...
      filterLevel: ({ fengMapApplyInteractive }) => fengMapApplyInteractive.filterLevel // 报警过滤级别
    }),
  ...
  methods: {
    ...
    sureStopAlarm() {
      ...
      this.alarmToBeSure(alarmArr)
        .then(res => {
          // 挂断报警（确认报警）成功，清除报警信息
          let index = this.alarmHelpStoreList.findIndex(item => {
            return sureAlarmInfo.alarmId === item.alarmId
          })
          index !== -1 && this.SPLICE_MAP2D_ALARM_LIST({type: 'alarmHelpList', index})
