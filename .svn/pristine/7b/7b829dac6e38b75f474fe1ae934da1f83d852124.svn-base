...
export default {
    ...
    return {
      ...
      resColumns: [
        ...
        },
        {
          ...
          sortable: true,
          minWidth: 160,
          sortMethod: (a, b, type) => { return type === 'asc' ? a.main.localeCompare(b.main) : b.main.localeCompare(a.main) },
          render: (h, params) => {
            let text = ''
  ...
  methods: {
    ...
    exportData() {
      let url = `/api/setting/resource/export/video?oid=${this.oid}&bigtype=${0}&type=${this.tabIndex}`
      let name = `${this.tabTypeFn()}-${this.$moment(new Date()).format('YYYYMMDDHHmmss')}.xlsx`
      download(url, name, () => {
      }, () => {this.errorMsg('下载失败!')})
    }
  },
