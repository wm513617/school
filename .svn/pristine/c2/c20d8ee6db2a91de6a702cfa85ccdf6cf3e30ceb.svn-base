...
<template>
  <dragBoxs @close="closeDragBox" :title="type === 'AttrPatrol' ? '巡更异常' : '单兵报警'" :eventType="'AttrPatrol' ? 'patrol' : 'single'" modelType="3D" @changeContext="scaleModalClick" :position="position">
...
export default {
      ...
      handler(newVal, oldVal) {
        switch (this.type) {
          case 'AttrPatrol':
            ...
            // this.currDealId = this.patrolData.currDealId
            // this.repairInfor = this.patrolData.repairInfor
            // this.showType = this.patrolData.showType
            // this.videoUrl = this.patrolData.videoUrl
  ...
  methods: {
    ...mapMutations(['SAVE_PATROL_RELY_STUATES', 'SAVE_SINGLE_LIST', 'SAVE_PATROL_STUATES', 'SAVE_PATROL_LIST', 'SAVE_ALARM_TYPE', 'SAVE_PATROL_ID', 'SAVE_SINGLE_ID', 'SAVE_MODAL_STATUS']),
    ...
    confirmAlarm(data) {
      ...
      this.confirmAlarms(param).then(resp => {
        ...
        this.closeDragBox()
        let map = this.pointAlarmData && this.pointAlarmData.map3D && this.pointAlarmData.map3D
        const bcode = map && map.bcode
        const isouter = map && map.isouter
        let obj = { pointIsouter: isouter, id: this.pointAlarmData._id, bcode: bcode, type: 'patrolAlarm' }
        this.confirmAlarmData(obj) // 取消地图点位闪烁
      }).catch(err => {
    ...
    cleanAlarm(uniqueId, data) {
      // console.log('danbing', this.alarmDetail)
      ...
      this.updatePatrolAlarm(param).then(resp => {
        ...
        this.SAVE_PATROL_ID(payload)
        let obj = { pointIsouter: true, id: uniqueId, bcode: '', type: 'singleAlarm', uid: this.alarmDetail.uid || this.alarmDetail._id }
        this.confirmAlarmData(obj) // 取消地图点位闪烁
      }).catch(err => {
        this.errorMsg(err.response.data.message)
        console.log('confirmAlarms error: ' + err)
