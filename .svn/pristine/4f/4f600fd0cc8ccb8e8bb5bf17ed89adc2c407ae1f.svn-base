<template>
  <div class="video-container">
    <!-- 傻逼用的 client\src\components\video\VideoFrame.vue -->
    <video-frame ref="frame" pluginType="record" :pluginCount="16" :clickInit="false" :defaultShowscreen="1" @update:state="setState" :slotHeight="56" @playEnd="finishPlay" @setShowscreen="num => $emit('screen', selectedShowscreen = num)" @activeChange="avtiveChange" @changePluginData="changePluginData" :bgColor="'#404040'" :fullScreenflag="fullScreenflag" :showBtn="false">
      <div class="playback">
        <div style="display: inline-block" class="leftIcon">
          ...
          <i class="icon iconfont icon-stop" :class="[btnStopDisable? 'disable' : '']" @click="stop(1)" title="停止"></i>
          <i class="icon iconfont icon-stop-all" @click="stopAll(1)" title="停止全部"></i>
          ...
          <div style="display:inline-block;">
            ...
            <div class="slider-box" style="width:80px;display:inline-block;vertical-align: top;" :style="{'display': state.isVolumeOpen?'inline-block':'none' }" @mouseenter.stop v-if="!showSpeed">
              <slider color="#20adff" v-model="volume" @change="_setVolume" :disabled="!clickOpenSound"></slider>
            </div>
          </div>
          <!-- 案件处理 -->
          <i class="cion iconfont icon-Location" v-if="type === 'caselist'" :class="{'disable': _isPlay?(caseEdit?false:!caseMarkingToogle):true}" title="起点标记"  @click="caseModalToggle(0)"></i>
          <i class="cion iconfont icon-Location" v-if="type === 'caselist'" :class="{'disable': _isPlay?(caseEdit?false:caseMarkingToogle):true}" title="终点标记"  @click="caseModalToggle(1)"></i>
        </div>

        <div style="float:right">
          <!-- 案件处理 -->
          <div class="dp-con save-case-time" v-if="type === 'caselist'">
            <i class="iconfont icon-delete" :class="{'disable': !caseEdit}" title="删除标记" @click="clickCaseTime('delete')"></i>
            <!-- <i class="iconfont icon-cunchu" :class="{'disable' : this.state.isStopped}" title="保存标记" @click="clickCaseTime('save')"></i> -->
          </div>
          <!-- 分段播放功能已实现 -->
          <!-- <div class="dp-con">
            <i class="icon iconfont" :class="{'disable': $parent.sourceType !== 1, 'icon-tuichufenduanhuifang': $parent.isSegment, 'icon-fenduanbofang': !$parent.isSegment}" @click="segmentPlay" :title="$parent.isSegment ? '关闭分段回放' : '分段回放'"></i>
            <ul v-show="segmentModal" class="segPlay">
              <i></i>
              <li>
                <TimePicker confirm type="timerange" :value="segmentTime" placement="bottom-end" size="small" format="HH:mm" placeholder="Select time" @on-change="segmentTimeChange" style="width: 168px"></TimePicker>
              </li>
...
// import { setVideoTime } from '../../../http/business/caseProcessing.api.js'
export default {
  components: {
    Slider
    // QuickToWall
  ...
  computed: {
    ...mapState({
      ...
      caseEdit: ({ dutyList }) => dutyList.caseEdit, // 案件处理 是否编辑
      caseMarkingToogle: ({ dutyList }) => dutyList.caseMarkingToogle // 【案件处理】【起、终标记】true【始】false【终】
    }),
    authInfo() {
      return {
        downLoad: true,
  ...
  methods: {
    ...
    ...mapMutations(['SET_RESOURCE', 'CLEAR_RESOURCE', 'ADD_DOWNLOADLIST', 'SET_CASE_MARKING_TOOGLE']),
    // 【案件处理】 保存/删除时间
    clickCaseTime(val) {
      if (this.state.isStopped) { return }
      // delete 删除 save 保存
      if (val === 'delete' && !this.caseEdit) {
        return
      }
      this.$emit('clickCase', val)
    },
    // 点击开始/结束标记
    caseModalToggle(val) {
      if (this._isPlay) {
        if (val) { // 1 end
          if (!this.caseEdit) {
            this.SET_CASE_MARKING_TOOGLE(Boolean(val))
          }
          this.$emit('isCase', 'end')
        } else { // 0 start
          if (!this.caseEdit) {
            this.SET_CASE_MARKING_TOOGLE(Boolean(val))
          }
          this.$emit('isCase', 'start')
        }
      }
    },
    setState(state) {
    ...
    stopAll(val) {
      ...
      // val 为当点击关闭按钮，自动清空同步播放syncResData数据
      if (val) {
        this.$emit('isStopAll')
      }
    },
    ...
    download() {
      ...
      let videoName =
        paramed.name +
        '(' +
        this.$moment(this.startTimeFormatted).format('YYYYMMDDHHmmss') +
        '-' +
        this.$moment(this.endTimeFormatted).format('YYYYMMDDHHmmss') +
        ')' +
        new Date().getTime() +
        '.' +
        videoType
    ...
    cutModalSave() {
      ...
      this.startTime = new Date(this.cutDate.startTime * 1000)
      this.endTime = new Date(this.cutDate.endTime * 1000)
      this.download()
      this.cutModalCannel()
    },
    // 剪切现在取消
...
.dp-con.save-case-time ul.display-scale {
  left: -72px;
  width: 110px;
  i {
    left: 102px;
