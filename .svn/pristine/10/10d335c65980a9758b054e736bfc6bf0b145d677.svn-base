 ...
 * @param {*} layerConfig 图层配置参数
 */
function transOneFeature(alarmHelp, layerConfig, pointMode = MAPMODE.point2D) {
  let feature = null
  let { _id, name, eid } = alarmHelp
  let point = JSON.parse(JSON.stringify(alarmHelp[pointMode]))
  point.mid = (pointMode === MAPMODE.point2D) ? point.mid : point.iid
  let symbol = getSymbolByEidType(eid)
  if (point.mid) { // 有图标信息时
    let icon = point.mid
