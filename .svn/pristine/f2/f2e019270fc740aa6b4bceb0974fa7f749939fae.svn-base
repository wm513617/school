      ...
      <div class="detail-right">
        ...
        <div v-if="tabItem === '0'" class="right-main">
          <Form :label-width="80" style="width: 300px;">
            <FormItem label="选择通道">
              <Select v-model="channel" placement="top" @on-change="toggleChannel(channel)">
          ...
          <div class="video">
            ...
            <!-- <SinglePawn v-else-if="warningDetail.eventType === '单兵一键报警' && isShowSinglePawn" :videoParam="singleInfo" style="height: 350px;"></SinglePawn> -->
            <SinglePawn v-else-if="warningDetail.eventType === '单兵一键报警' && isShowSinglePawn" style="height: 400px;" :windowAdapter="[1, 1, true]" :configuration="configuration" :defaultPane="1" ref="plugChildren"></SinglePawn>
            <SinglePawn v-else style="height: 400px;" :configuration="configuration" :defaultPane="1" ref="recordVideo"></SinglePawn>
          </div>
        </div>
...
import SinglePawn from 'components/videoComponentsNew/playbackPlugin'
...
export default {
  ...
  data() {
    return {
      ...
      isShowSinglePawn: false, // 单兵信息获取完成后变为true
  ...
  computed: {
    ...mapState({
      channelInfo: ({ warningCount }) => warningCount.channel
    })
  },
  watch: {
    // channelInfo: {
    //   handler(val) {
    //     this.playRecordVideo(val)
    //   }
    // }
  },
  created() {
  ...
  methods: {
    ...
    playSingleVideo() {
      if (this.warningDetail.eventType === '单兵一键报警') {
        if (this.warningDetail.recordId) {
          this.getSingleInfo({id: this.warningDetail.recordId}).then(res => {
            /* this.singleInfo = {
            ...
            const param = [{
              ...
              dsId: res.data.storage,
              startTime: res.data.time,
              endTime: parseInt(res.data.time) + 600, // start + 10min
              page: 1,
              rows: 50,
              videoSegmentsIndex: 0,
              name: '一键报警'
            }]
            this.isShowSinglePawn = true
            this.$nextTick(() => {
              this.$refs.plugChildren.openPlayback(param)
            })
          }).catch(() => {
            this.isShowSinglePawn = true
          })
      ...
      } else if (this.warningDetail.eventType !== '单兵一键报警' && this.warningDetail.eventType !== '巡更异常上报') {
        this.playRecordVideo(this.channelInfo)
      }
    },
    playRecordVideo(val) {
      const param = [{
        ...
        devPort: val.devPort,
        channel: val.channel,
        eventType: val.eventType ? val.eventType : ['all'],
        typeName: '',
        typeContent: '',
        startTime: parseInt(val.startTime),
        endTime: parseInt(val.endTime),
        streamType: 'all',
        page: 1,
        rows: 50,
      ...
      this.$nextTick(() => {
        this.$refs.recordVideo.openPlayback(param)
      })
    },
    fullscreen() {
      ...
      if (data) {
        this.channelInfoList.forEach((item, index) => {
          if (data === item.channelName) {
            if (this.warningDetail.eventType === '消防报警') {
              item.eventType = [41]
            }
