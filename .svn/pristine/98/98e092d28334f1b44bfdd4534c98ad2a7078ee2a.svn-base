  ...
  <div class="compare-record-search">
    ...
    <div class="compare-condition">
      <Form :model="condition" inline :label-width="80">
        ...
        <Form-item label="姓名" class="firstRow">
          <Row>
            <Col span="24">
        ...
        <Form-item label="性别" class="firstRow">
          <Row>
            <Col span="24">
        ...
        <Form-item label="相似度 >" class="firstRow">
          <Row>
            <Col span="22">
            <Input-number :max="100" :min="0" v-model="condition.similarity" style="width: 100%;height: 32px;"></Input-number>
            </Col>%
          </Row>
        </Form-item>
        <Form-item label="年龄范围" class="firstRow">
          <Row>
            <Col span="10">
            <Input-number :max="100" :min="1" v-model="condition.min" style="width: 100%;height: 32px;"></Input-number>
            </Col>
        ...
        <Form-item label="时间段选择" class="secondRow">
          <Row>
            <Col span="22">
            <Date-picker :options="{
                  disabledDate (date) {
        ...
        <Form-item>
          ...
          <Button type="ghost" @click="ExportCsv" style="width: 100px;">导出</Button>
        </Form-item>
      </Form>
    </div>
    <!--内容显示框-->
    <div class="result-show">
      <Tabs value="thumbnail">
        <TabPane label="缩略图" name="thumbnail" style="overflow:hidden;">
          ...
          <ul class="thumbnail-list clearfix" v-if="resultList.length > 0">
            <li v-for="item of contrastList" :key="item">
        ...
        <TabPane label="列表" name="list">
          <Row>
            <Col span="22">
            <Table style="height: 432px;" size="small" :row-class-name="rowClassName" highlight-row :columns="columns" :data="resultList" @on-row-click="showRowInfo"></Table>
      ...
      <div class="operate">
        ...
        <Button type="ghost" @click="listSort($event, 'simi')">按相似度排序</Button>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    retrieval() {
      ...
      this.condition.similarity &&
        (params.similarity = this.condition.similarity)
    ...
    ExportCsv() {
      ...
      this.searchCompare(payload).then(res => {
        var exportData = res.data.map(item => {
          return {
            snapshotTime:
              '~' +
              this.$moment
                .unix(Number(item.snapshotTime))
                .format('YYYY-MM-DD HH:mm:ss'),
    ...
    handleAge() {
      ...
      // 限定第二个年龄值小于第一个则等于第一个
      this.condition.max < this.condition.min &&
        (this.condition.max = this.condition.min)
    },
    handleSimilarity() {
